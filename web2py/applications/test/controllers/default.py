...
def index():
    ...
    checklists = None
    public_checklists = None
    if auth.user is not None:
        checklists = db((db.checklist.user_email == auth.user.email) | (db.checklist.is_public == True)).select()
        return dict(checklists=checklists)
    else:
        public_checklists = db(db.checklist.is_public == True).select()
        return dict(public_checklists= public_checklists)
...
def toggle_public():
    if request.args(0) is not None:
        ...
        elem = db(q).select().first()
        if elem.is_public:
            db(q).update(is_public=False)
            logger.info(elem.is_public)
        else:
